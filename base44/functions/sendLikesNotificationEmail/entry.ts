...
Deno.serve(async (req) => {
  try {
    ...
    const receiverUser = allUsers.find(u => u.id === swiped_id);
    if (receiverUser?.notify_likes === false) {
      console.log(`🔕 User ${swiped_id} has disabled likes notifications, skipping email`);
