...
export default function DiscoverFilters({ filters, onChange }) {
  ...
  const [local, setLocal] = useState({ kosher: 'all', shabbat: 'all', hasApartment: 'all', ...filters, maxAge: filters.maxAge ?? 60 });
  const [cityInput, setCityInput] = useState("");
  const [citySuggestions, setCitySuggestions] = useState([]);

  useEffect(() => {
    const handler = () => { setLocal({ kosher: 'all', shabbat: 'all', hasApartment: 'all', ...filters }); setCityInput(""); setOpen(true); };
  ...
  const activeCount = [
    ...
    local.hasApartment && local.hasApartment !== 'all',
  ...
  const reset = () => {
    const defaults = { cities: [], minBudget: 0, maxBudget: 10000, minAge: 18, maxAge: 60, kosher: 'all', shabbat: 'all', hasApartment: 'all' };
    setLocal(defaults);
    setCityInput("");
    onChange(defaults);
    setOpen(false);
        ...
        {open && (
          ...
          >
            ...
            >
              ...
              {/* Has Apartment */}
              <div className="mb-6">
                <label className="text-sm font-bold text-gray-700 mb-2 block">{t("has_apartment_filter_label")}</label>
                <div className="flex bg-gray-100 p-1 rounded-xl">
                  {[
                    { v: 'all', l: t('all') },
                    { v: 'yes', l: t('has_apartment_yes') },
                  ].map(opt => (
                    <button
                      key={opt.v}
                      onClick={() => setLocal(prev => ({ ...prev, hasApartment: opt.v }))}
                      className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${local.hasApartment === opt.v ? 'bg-white shadow-sm text-black' : 'text-gray-400'}`}
                    >
                      {opt.l}
                    </button>
                  ))}
