  ...
  useEffect(() => {
    const trackStep = async () => {
      try {
        ...
        if (simulatorMode) {return true;}if (formData.current_status === 'has_apartment') {const apartmentPhotoCount = formData.apartment_photos?.filter((p) => p).length || 0;return apartmentPhotoCount >= 3 && formData.existing_roommates >= 0 && formData.apartment_total_budget > 0;}return true;
      ...
      <div className="w-full max-w-md flex flex-col h-[85vh]">
        ...
        <div className="flex-1 relative">
            <Step step={1} currentStep={step} title="בואו נכיר!">
                <p className="text-center mb-4 text-base" style={{ color: '#FFB29D' }}>ספר/י לנו קצת על עצמך</p>
            ...
            <Step step={3} currentStep={step} title="העדפות">
                <div className="space-y-3 text-right">
                    {/* אני מחפש/ת */}
                    <div>
                        <label className="text-sm font-bold block py-1" style={{ color: '#FA3803' }}>אני מחפש/ת</label>
