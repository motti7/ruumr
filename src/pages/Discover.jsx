...
export default function DiscoverPage() {
  ...
  const [filters, setFilters] = useState({ cities: [], minBudget: 0, maxBudget: 10000, minAge: 18, maxAge: 60, kosher: 'all', shabbat: 'all', housingStatus: 'all' });
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (filters.cities.length > 0 || filters.maxBudget < 10000 || filters.minAge > 18 || filters.maxAge < 60 || (filters.kosher && filters.kosher !== 'all') || (filters.shabbat && filters.shabbat !== 'all') || (filters.housingStatus && filters.housingStatus !== 'all')) {
        displayProfiles = availableProfiles.filter(p => {
          ...
          if (filters.housingStatus && filters.housingStatus !== 'all') {
            if (filters.housingStatus === 'has_apartment' && p.current_status !== 'has_apartment') return false;
            if (filters.housingStatus === 'seeking_apartment' && p.current_status === 'has_apartment') return false;
          }
          return true;
        });
      }
  ...
  const applyFilters = (newFiltersArg) => {
    ...
    setAllProfiles(currentAll => {
      const filtered = currentAll.filter(p => {
        ...
        if (newFilters.housingStatus && newFilters.housingStatus !== 'all') {
          if (newFilters.housingStatus === 'has_apartment' && p.current_status !== 'has_apartment') return false;
          if (newFilters.housingStatus === 'seeking_apartment' && p.current_status === 'has_apartment') return false;
        }
        return true;
      });
      setCurrentIndex(0);
